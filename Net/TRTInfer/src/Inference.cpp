...
namespace TRTInferV1
{
    ...
    void TRTInfer::decode_output(std::vector<DetectionObj> &res, cv::Mat &frame, float *pdata, float &obj_threshold, float &confidence_threshold, float &nms_threshold) {
        ...
        padw /= 2;
        padh /= 2;
        //int n = 0, q = 0, i = 0, j = 0, row_ind = 0, k = 0; /// xmin,ymin,xmax,ymax,box_score, class_score
        //int a = this->output_dims.d[2];

        //float *bufferPtr = pdata;
        const int stride = this->output_dims.d[2];
        for (int boxId = 0 ; boxId < this->output_dims.d[2]; boxId++)
        {
            // printf("///\n");
            float maxScore = 0;
            int maxScoreID = 0;
            for (int classID = 0; classID < this->output_dims.d[1] - 4; classID++)
            {
                // printf("%f\n", pdata[(4 + classID) * stride + boxId]);
                if (pdata[(4 + classID) * stride + boxId] > maxScore)
                {
                    maxScoreID = classID;
                    maxScore = pdata[(4 + classID) * stride + boxId];
                }
            }
            // printf("---\n%f|%f", maxScore, std::max(obj_threshold, confidence_threshold));
            if (maxScore > std::max(obj_threshold, confidence_threshold) && maxScore != 1)
            {
                //cou++;
                float cx = pdata[0 * stride + boxId]; /// cx
                float cy = pdata[1 * stride + boxId]; /// cy
                float w = pdata[2 * stride + boxId];  /// w
                float h = pdata[3 * stride + boxId];  /// h
                ...
                {
                    res.emplace_back(DetectionObj{maxScoreID, maxScore, xmin, ymin, xmax, ymax});
                    // printf("%d\n", res.size());
    ...
    void TRTInfer::setDevice(const int device) {
        CHECK(cudaSetDevice(device));
    }
    ...
    bool TRTInfer::initModule(const std::string engine_file_path, const int batch_size, const int num_classes)
    {
        ...
        else if (this->input_dims.d[2] == 640)
        {
        ...
        {
            this->gLogger.log(ILogger::Severity::kERROR, "The onnx is not 640*640 or 1280*1280");
            assert(0); // The Input is not 640*640 or 1280*1280
            return false;
        }
