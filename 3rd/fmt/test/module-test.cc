// Formatting library for C++ - modules tests
//
...
// that are not essential to users of the modules.
...
// an implicitly exported namespace must be visible [modules.interface]/2.2
...
// the non-exported namespace 'detail' must be invisible [modules.interface]/2
TEST(module_test, detail_namespace) {
  EXPECT_TRUE(fmt::namespace_detail_invisible());
}

// macros must not be imported from a *named* modules  [cpp.import]/5.1
...
#else
  fmt::ostream out = fmt::output_file("modules-test", fmt::buffer_size = 1);
  out.close();
#endif
}
